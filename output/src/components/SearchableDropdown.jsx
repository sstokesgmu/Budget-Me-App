...
export default function SearchableDropdown({ options, handleChange }) {
    options.forEach((element, index, array) => { array[index] = typeof element === 'string' ? element : String(element); });
    ...
    return (<section className={classes.container}>
      <div className={classes.searchBarContainer}>
        ...
        <input type="text" name="choosingAnAccount" placeholder="Enter Account here" onChange={(e) => { setQuery(e.target.value); }}>
        </input>
      </div>
      <div className={classes.list}>
            {filter(options).map((option, index) => {
            return (<div key={`account ${index}`} onClick={() => selectAnOption(option)}>
                            {option}
                        </div>);
        })}
      </div>
